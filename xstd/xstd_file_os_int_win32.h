...
static int _fosint_win32_flush(void *stream)
{
    _Win32File *f = (_Win32File *)stream;
    return FlushFileBuffers(f->h);
}

static int _fosint_win32_eof(void *stream)
{
    _Win32File *f = (_Win32File *)stream;
    if (!f || !f->h)
        return 1;
...
static u64 _fosint_win32_size(void *stream)
{
    _Win32File *f = (_Win32File *)stream;
    if (!f || !f->h)
        return 0;

    _w32_large_integer size;
...
static const _FileOsInterface _file_os_int_win32 = {
    ...
    .flush = _fosint_win32_flush,
    .eof = _fosint_win32_eof,
    .size = _fosint_win32_size,
};
